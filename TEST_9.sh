...
rm -f plumed.dat
touch plumed.dat
echo 'mat: TOPOLOGY_MATRIX NOPBC NODES=1-2 ATOMS=3-4 BIN_SIZE=1.0 CYLINDER_SWITCH={RATIONAL  D_0=0.0 R_0=0.001 D_MAX=0.002} SWITCH={RATIONAL D_0=3.1 R_0=0.01 D_MAX=3.12} RADIUS={RATIONAL D_0=0.5 R_0=0.000001 D_MAX=0.6} SIGMA=1.0 KERNEL=triangular DENSITY_THRESHOLD={RATIONAL D_0=100.0 R_0=10.0 D_MAX=150.0}' >> plumed.dat
...
rm -f plumed.dat
touch plumed.dat
echo 'mat: TOPOLOGY_MATRIX NOPBC NODES=1-2 ATOMS=3-4 BIN_SIZE=1.0 CYLINDER_SWITCH={RATIONAL  D_0=0.0 R_0=0.001 D_MAX=0.002} SWITCH={RATIONAL D_0=2.9 R_0=0.01 D_MAX=2.92} RADIUS={RATIONAL D_0=0.5 R_0=0.000001 D_MAX=0.6} SIGMA=1.0 KERNEL=triangular DENSITY_THRESHOLD={RATIONAL D_0=100.0 R_0=10.0 D_MAX=150.0}' >> plumed.dat
echo 'DFSCLUSTERING MATRIX=mat LABEL=ss1' >> plumed.dat
echo ' ' >> plumed.dat
echo 'OUTPUT_CLUSTER CLUSTERS=ss1 CLUSTER=1 FILE=Cluster_1.dat' >> plumed.dat
echo 'OUTPUT_CLUSTER CLUSTERS=ss1 CLUSTER=2 FILE=Cluster_2.dat' >> plumed.dat
...
if diff -b Correct_Result.dat k >/dev/null ; then
  echo -e '\e[42m\e[97m    TEST-9    PASSED SUCCESSFULLY    \033[0m'
else
