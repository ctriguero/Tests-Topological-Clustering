...
echo "* Checks if it picks the connected path even if it is longer way through the PBC"
echo "* [THIS WILL FAIL AS IS NOT IMPLEMENTED IN PLUMED, BUT IDEALLY IT SHOULD BE IMPLEMENTED]"
rm -f kk.dat

# Create the system to analize file TEST_1.xyz
touch TEST_1.xyz
echo '7'                       >> TEST_1.xyz
echo '11.95 1.0 1.0'            >> TEST_1.xyz
...
if diff -b Correct_Result.dat k >/dev/null ; then
  echo -e '\e[42m\e[97m    TEST-11    PASSED SUCCESSFULLY    \033[0m'
else
  echo -e '\e[41m\e[97m    TEST-11    FAILED    \033[0m'
fi
#rm -f bck.* k
rm -f *~
rm -f Cluster_*.dat
