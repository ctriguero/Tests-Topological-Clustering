...
echo 'Ar   -1.0    0.0    0.0' >> TEST_1.xyz
echo 'Ne    0.0    1.0    0.0' >> TEST_1.xyz
...
rm -f plumed.dat
touch plumed.dat
echo 'mat: TOPOLOGY_MATRIX NOPBC NODES=1-2 ATOMS=3-3 BIN_SIZE=2.0 CYLINDER_SWITCH={RATIONAL  D_0=0.0 R_0=0.001 D_MAX=0.002} SWITCH={RATIONAL D_0=20.0 R_0=10.0 D_MAX=50.0} RADIUS={RATIONAL D_0=0.5 R_0=0.000001 D_MAX=0.6} SIGMA=0.01 KERNEL=triangular DENSITY_THRESHOLD={RATIONAL D_0=0.0 R_0=0.000001 D_MAX=0.000002}' >> plumed.dat
...
rm -f plumed.dat
touch plumed.dat
echo 'mat: TOPOLOGY_MATRIX NOPBC NODES=1-2 ATOMS=3-3 BIN_SIZE=2.0 CYLINDER_SWITCH={RATIONAL D_0=0.0 R_0=0.001 D_MAX=0.002} SWITCH={RATIONAL D_0=20.0 R_0=10.0 D_MAX=50.0} RADIUS={RATIONAL D_0=1.000001 R_0=0.000001 D_MAX=1.2} SIGMA=0.01 KERNEL=triangular DENSITY_THRESHOLD={RATIONAL D_0=0.0 R_0=0.000001 D_MAX=0.000002}' >> plumed.dat
echo 'DFSCLUSTERING MATRIX=mat LABEL=ss1' >> plumed.dat
echo ' ' >> plumed.dat
echo 'OUTPUT_CLUSTER CLUSTERS=ss1 CLUSTER=1 FILE=Cluster_1.dat' >> plumed.dat
echo 'OUTPUT_CLUSTER CLUSTERS=ss1 CLUSTER=2 FILE=Cluster_2.dat' >> plumed.dat
...
if diff -b Correct_Result.dat k >/dev/null ; then
  echo -e '\e[42m\e[97m    TEST-7    PASSED SUCCESSFULLY    \033[0m'
else
  echo -e '\e[41m\e[97m    TEST-7    FAILED    \033[0m'
fi
rm -f bck.* k
rm -f *~
rm -f Cluster_*.dat
