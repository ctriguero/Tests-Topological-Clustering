...
touch TEST_1.xyz
echo '3'                       >> TEST_1.xyz
echo '3.0 1.0 1.0'             >> TEST_1.xyz
echo 'Ar    1.0    0.0    0.0' >> TEST_1.xyz
echo 'Ar   -1.0    0.0    0.0' >> TEST_1.xyz
...
rm -f plumed.dat
touch plumed.dat
echo 'mat: TOPOLOGY_MATRIX NOPBC NODES=1-2 ATOMS=3-3 BIN_SIZE=2.0 CYLINDER_SWITCH={RATIONAL  R_0=0.000001 D_MAX=0.1} SWITCH={RATIONAL D_0=10.0 R_0=0.5 D_MAX=15.0} RADIUS={RATIONAL D_0=0.5 R_0=0.000001 D_MAX=0.6} SIGMA=1.0 KERNEL=triangular DENSITY_THRESHOLD={RATIONAL D_0=0.56 R_0=0.000001 D_MAX=1.5}' >> plumed.dat
...
rm -f plumed.dat
touch plumed.dat
echo 'mat: TOPOLOGY_MATRIX NOPBC NODES=1-2 ATOMS=3-3 BIN_SIZE=2.0 CYLINDER_SWITCH={RATIONAL  R_0=0.000001 D_MAX=0.1} SWITCH={RATIONAL D_0=10.0 R_0=0.5 D_MAX=15.0} RADIUS={RATIONAL D_0=0.5 R_0=0.000001 D_MAX=0.6} SIGMA=1.0 KERNEL=triangular DENSITY_THRESHOLD={RATIONAL D_0=0.54 R_0=0.000001 D_MAX=1.5}' >> plumed.dat
echo 'DFSCLUSTERING MATRIX=mat LABEL=ss1' >> plumed.dat
echo ' ' >> plumed.dat
echo 'OUTPUT_CLUSTER CLUSTERS=ss1 CLUSTER=1 FILE=Cluster_1.dat' >> plumed.dat
echo 'OUTPUT_CLUSTER CLUSTERS=ss1 CLUSTER=2 FILE=Cluster_2.dat' >> plumed.dat
...
if diff -b Correct_Result.dat k >/dev/null ; then
  echo -e '\e[42m\e[97m    TEST-4    PASSED SUCCESSFULLY    \033[0m'
else
  echo -e '\e[41m\e[97m    TEST-4    FAILED    \033[0m'
fi
rm -f bck.* k
rm -f *~
rm -f Cluster_*.dat
